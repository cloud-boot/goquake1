...
func TestPushEntity_SolidNotShortCircuit(t *testing.T) {
	...
	out, err := PushEntity(in, makeWorldWithWall(), nil)
	if err != nil {
...
func TestPushEntity_NoClipShortCircuit(t *testing.T) {
	in := PushEntityIn{
		...
		MoveType: server.MoveTypeNoClip,
		Solid:    server.SolidBBox, // still SOLID but NoClip wins
	}
	out, err := PushEntity(in, makeWorldWithWall(), nil)
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
...
func TestPushEntity_CleanThroughEmptyWorld(t *testing.T) {
	...
	out, err := PushEntity(in, makeEmptyWorld(), nil)
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if out.Trace.Fraction != 1.0 {
...
func TestPushEntity_HitsCandidate(t *testing.T) {
	...
	out, err := PushEntity(in, makeEmptyWorld(), []Target{candidate})
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if out.Trace.Fraction >= 1.0 {
...
//
// Observation: a Minkowski-diff boxhull trace that runs entirely
// INSIDE the candidate's widened box reports AllSolid + Fraction=1
// (start and end both classified as solid). [TraceMove]'s running-
// trace rule promotes such a stack-trace to the result and sets
// EntityIdx -- which is what we use to distinguish "candidate saw
// the trace" (Fly: no, EntityIdx=-1; FlyMissile: yes, EntityIdx=0).
func TestPushEntity_FlyMissileWidensCandidateBounds(t *testing.T) {
	...
	makeIn := func(mt server.MoveType) PushEntityIn {
		return PushEntityIn{
	...
	flyOut, err := PushEntity(makeIn(server.MoveTypeFly), makeEmptyWorld(), []Target{candidate})
	if err != nil {
		t.Fatalf("Fly: %v", err)
	}
	...
	// EntityIdx = 0 (promoted by [TraceMove]'s running rule).
	missileOut, err := PushEntity(makeIn(server.MoveTypeFlyMissile), makeEmptyWorld(), []Target{candidate})
	if err != nil {
...
func TestPushEntity_FlyMissileNoCandidates(t *testing.T) {
	in := PushEntityIn{
		Origin:   [3]float32{-10, 0, 0},
		Push:     [3]float32{5, 0, 0},
		MoveType: server.MoveTypeFlyMissile,
		Solid:    server.SolidBBox,
	}
	out, err := PushEntity(in, makeEmptyWorld(), nil)
	if err != nil {
...
func TestPushEntity_WorldErrorPropagates(t *testing.T) {
	...
	out, err := PushEntity(in, pushEntityCorruptWorld(), nil)
	if err == nil {
...
// pushEntityCorruptWorld returns a brushmodel whose hull 0 references
// an out-of-range plane index -- [TraceMove] returns ErrBadPlaneIndex
// which PushEntity is expected to propagate.
func pushEntityCorruptWorld() *model.BrushModel {
	bm := &model.BrushModel{}
